...
from src.models.schemas import User, ProcessingResult, PyObjectId
...
class AnalyzeRequest(BaseModel):
    ...
    questions: Optional[List[dict]] = Field(None, description="Optional custom questions. If null, uses defaults.")
...
async def analyze_asset(request: AnalyzeRequest, current_user: User = Depends(get_current_user), db=Depends(get_database)):
    ...
    if not encrypted_key:
        raise HTTPException(status_code=400, detail="Gemini API Key not configured for this user. Please save it in your profile first.")
...
async def get_general_history(
    ticker: Optional[str] = Query(None, description="Filtro opcional por ticker"),
    type: Optional[str] = Query(None, description="Filtro opcional por tipo"),
    start_date: Optional[str] = Query(None, description="Filtro inicial de data (ISO)"),
...
async def get_specific_history(type: str, ticker: str, current_user: User = Depends(get_current_user), db=Depends(get_database)):
...
async def get_default_questions():
    return {
        "stocks": DEFAULT_STOCKS_QUESTIONS,
        "real_estate_funds": DEFAULT_FUNDS_QUESTIONS
    }
