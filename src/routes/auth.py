...
from src.core.security import create_access_token, get_current_user, verify_google_token
...
async def save_gemini_key(
    request: GeminiKeyRequest, current_user: User = Depends(get_current_user), db=Depends(get_database)
):
...
async def delete_api_key(current_user: User = Depends(get_current_user), db=Depends(get_database)):
    await db["users"].update_one({"_id": current_user["_id"]}, {"$unset": {"encrypted_gemini_key": ""}})
